...
export const ServiceSidebar: React.FC<ServiceSidebarProps> = React.memo(({ model, onCTAClick, serviceSlug }) => {
  ...
  return (
    <>
      ...
      >
        <div
          className="bg-gradient-to-br from-primary-600 to-primary-800 rounded-l-lg border-t border-b border-l border-primary-700 p-5 h-full flex flex-col"
          style={{
            height: '100%',
            maxHeight: '100%',
            overflow: 'hidden',
        ...
        >
          ...
          <button
            onClick={() => navigate('/')}
            className="w-full mb-4 bg-transparent hover:bg-primary-700/20 text-white font-semibold py-2.5 px-3 rounded-lg transition-all duration-200 flex items-center justify-center gap-3 flex-shrink-0"
            aria-label="Back to home"
          >
            <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5 text-primary-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden" style={{ overflow: 'hidden' }}>
            {/* Video Section */}
            <div ref={videoRef} className="mb-4 overflow-hidden flex-shrink-0 border-2 border-white rounded-lg">
            ...
            {/* What Will You Get */}
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden" style={{ overflow: 'hidden' }}>
              <h4 className="text-lg font-bold text-white mb-3 flex-shrink-0">What Will You Get:</h4>
              <div className="flex-1 overflow-hidden" style={{ overflow: 'hidden' }}>
                <ul className="space-y-2 pr-2 pb-1">
          ...
          <div className="mt-4 flex-shrink-0">
            <button
              onClick={onCTAClick}
              className="w-full bg-white hover:bg-gray-50 text-primary-600 font-bold py-2.5 px-4 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg border-2 border-white flex items-center justify-center gap-2"
            >
              {model.buttonText}
            </button>
          </div>
